...
class ShallowFusion(nn.Module):
    def __init__(self, inter_size, out_size):
        ...
        # LayerNorm operates on the last dimension (channel dimension)
        self.norm = nn.LayerNorm(inter_size*8)
        self.hidden = nn.Linear(inter_size*8, inter_size)
...
class TailBranch(nn.Module):
    def __init__(self, input_size, tail_hidden_size, tail_num_layers, inter_size, activation, 
        ...
        self.in_mod = Conv1d_permute(self.input_size, self.inter_size, kernel_size=1)
        #self.mod2 = nn.Linear(self.inter_size//8, self.inter_size)
        self.act = nn.GELU()
        ...
        layers.append(SequenceLayer(
            d_in=inter_size,
            d_state=tail_hidden_size*12 , #factor 3 before
            d_out=inter_size*3,
            input_bias=True,
            output_bias=True,
            complex_input = False,
        ...
        layers.append(SequenceLayer(
            d_in=inter_size*3,
            d_state=tail_hidden_size*10 , #factor 3 before
            d_out=inter_size*4,
            input_bias=True,
            output_bias=True,
            complex_input = False,
        ...
        layers.append(SequenceLayer(
            d_in=inter_size*4,
            d_state=tail_hidden_size*8 , #factor 3 before
            d_out=inter_size*5,
            input_bias=True,
            output_bias=True,
            complex_input =False,
        ...
        layers.append(SequenceLayer(
            d_in=inter_size*5,
            d_state=tail_hidden_size*6 , #factor 3 before
            d_out=inter_size*6,
            input_bias=True,
            output_bias=True,
            complex_input = False,
            complex_output=False,
            trainable_SkipLayer=True, #False
            act=activation,
            pre_conv=False
        ))
        ...
        self.skip = Conv1d_permute(inter_size, inter_size, kernel_size = 1)
        #self.proj = nn.Linear(inter_size*8, inter_size)
    ...
    def forward(self, x):
        ...
        #x1 = self.mod2(x1)
        x1 = self.ssm_layers(x1 + self.skip(res))
        #x1 = self.proj(x1)
        x2 = self.shallow_fusion(x1)
...
class CONV_SSM_L(nn.Module):
    def __init__(self, 
        ...
        super(CONV_SSM_L, self).__init__()
